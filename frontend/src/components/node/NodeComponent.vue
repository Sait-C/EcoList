...
const nodeSpacing = computed(() => {
  const baseSpacing = 100; // Base spacing between nodes
  const topicCount = Math.max(...props.nodes.map(node => node.topics.length));
  return baseSpacing + (topicCount * 50); // Increase spacing based on max topics
});

const containerDimensions = computed(() => {
  const maxTopics = Math.max(...props.nodes.map(node => node.topics.length));
  const topicHeight = 50; // Height of each topic box
  const topicSpacing = 90; // Base spacing for topics
  const nodeHeight = 80; // Height of node
  const minHeight = 800; // Minimum container height
...
const calculateTopicPosition = (index, totalTopics) => {
  const boxWidth = 160; // Increased width for topic box
  const gap = 20; // Gap between boxes
  const totalWidth = (boxWidth + gap) * totalTopics;
  const startX = -totalWidth / 4; // This centers all topics relative to node
